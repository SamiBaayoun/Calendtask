...
/**
 * Store dérivé : Todos filtrés par recherche ET sans date (pour la sidebar)
 */
export const filteredTodosWithoutDate = derived([todos, searchQuery], ([$todos, $searchQuery]) => {
  // D'abord filtrer par absence de date
  const todosWithoutDate = $todos.filter(todo => !todo.date);

  // Puis appliquer le filtre de recherche
  if (!$searchQuery || $searchQuery.trim() === '') {
    return todosWithoutDate;
  }

  const query = $searchQuery.toLowerCase();
  return todosWithoutDate.filter(todo => {
    // Recherche dans le texte
    if (todo.text.toLowerCase().includes(query)) return true;
...
/**
 * Store dérivé : Grouper les todos par tags (uniquement les todos sans date pour la sidebar)
 */
export const tagGroupsWithoutDate = derived(filteredTodosWithoutDate, ($filteredTodosWithoutDate) => {
  const groups = new Map<string, TagGroup>();
  ...
  // Grouper par tag
  $filteredTodosWithoutDate.forEach((todo) => {
    if (todo.tags.length === 0) {
      groups.get('_no_tag')!.todos.push(todo);
    } else {
      todo.tags.forEach((tag) => {
